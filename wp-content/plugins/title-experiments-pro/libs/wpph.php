...
class WPPH {
	...
	private $_support_enabled = FALSE;
	private $license;
	private $trial;
	...
	function __construct($slug, $api_key, $options = array()) {
		...
		if(isset($this->options['enable_support']) && $this->options['enable_support']) {
			$this->enable_support();
		}

		if(isset($this->options['enable_license']) && $this->options['enable_license']) {
			$this->enable_license();
		}

		add_action('admin_head', array($this,'flash'));
	}
	...
	public function flash($msg=NULL) {
		if($msg) {
			if(!isset($_SESSION['_wpph_flash'])) {
				$_SESSION['_wpph_flash'] = array();
			}
			$_SESSION['_wpph_flash'][] = $msg;
		} else {
			if(isset($_SESSION['_wpph_flash']) && !empty($_SESSION['_wpph_flash'])) {
				$messages = "";
				foreach($_SESSION['_wpph_flash'] as $msg) {
					$messages .= $this->_alert($msg);
				}
				$messages = addcslashes($messages, '"');
				echo <<< EOT
				<script type="text/javascript">
					jQuery(document).ready(function() {
						jQuery("#wpbody-content").prepend("$messages");
					});
				</script>
EOT;
				unset($_SESSION['_wpph_flash']);
			}
		}
	}
	...
	public function license_menu() {
		...
		if(isset($_POST['license'])) {
			...
			if($valid) {
				...
				$messages .= $this->_alert("Your license key has not been set." . $this->lastError);
			}
		} else {
			$valid = $this->check_license() && !$this->is_trial();
		}
...
EOT;
			...
			// These are for upgrades
			// $url = $this->api_url_base. "/plugin/purchase/form/{$license}?k=" . $this->api_key . "&s=" . get_site_url()."&TB_iframe=true&width=985&height=600";
			// echo <<< EOT
					// <a href="$url" class="thickbox button button-primary" title="Purchase Plugin License">Purchase License</a>
// EOT;
			} else if($this->options['enable_license_purchase']) {
				$types = $this->get_license_types();
				$url = $this->api_url_base. "/plugin/purchase/form?k=" . $this->api_key . "&s=" . get_site_url()."&TB_iframe=true&width=985&height=600";
				echo <<< EOT
					<a href="$url" class="thickbox button button-primary" title="Purchase Plugin License">Purchase License</a>
EOT;
		}
...
EOT;
	...
	private function _enable_support() {
		...
		if(!isset($admin_page_hooks[$this->options['support_menu_slug']])) {
			// readd the action to ensure that it gets called
			add_action( 'admin_menu', array($this,'enable_support'), 1000);
			return;
		}
		if($this->_support_enabled) return;
		$this->_support_enabled = true;
	...
	public function support_open_ticket() {
		...
		if(isset($_POST['submit'])) {
			...
			if(isset($_POST['ticket']["details"])) {
				...
				ob_end_clean();
				$request_string['data'] = json_encode(array(
...
EOT;
	...
	public function __check_for_updates($checked_data) {
		...
		if (is_object($response) && property_exists($response, "version") && $response->version) // Feed the update data into WP updater
