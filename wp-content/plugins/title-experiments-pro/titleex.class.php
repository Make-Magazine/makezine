...
class TitleEx {
	...
	function enqueue() {
		wp_enqueue_script('titleex', plugins_url('js/titleexpro.js',__FILE__),array("jquery"), '0.8.8');
	...
	function should_run_experiment() {
	...
	function extra_js_attrs($title, $id, $ajax, $viewed) {
		$sample_size = $this->get_option("wpex_sample_size", 1);
	...
	function do_freeze() {
		...
		if($freeze_after != "-1") {
			...
			if((current_time("timestamp") - 60*60) > $last_freeze) {
				update_option("wpex_last_freeze", current_time("timestamp"));
				$date = date("Y-m-d", strtotime($freeze_after));
				$sql = "select ID, post_modified from ".$wpdb->prefix."posts WHERE ID in (select post_id FROM ".$wpex->titles_tbl.") AND post_modified < '$date';";
