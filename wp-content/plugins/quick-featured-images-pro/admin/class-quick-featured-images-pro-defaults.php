...
class Quick_Featured_Images_Pro_Defaults {
	...
	/**
	 * Options for 'first image' selection
	 *
	 * @since    3.6 pro
	 *
	 * @var     string
	...
	*/
	...
	private function get_thumb_id ( $post_id, $rule ) {
		...
		$id = $this->get_single_id( $rule );
		
		if ( 'searchterm' == $rule[ 'taxonomy' ] ) {
			if ( false !== strpos( get_the_title( $post_id ), $rule[ 'matchterm' ] ) ) {
				return $id;
			} else {
				return 0;
			}
	 ...
	 * @since    3.8 pro
	 */
	private function get_single_id ( $rule ) {

		$id = 0;
		
		if ( $rule[ 'ids' ] and is_array( $rule[ 'ids' ] ) ) {
			// resort values randomly
			shuffle( $rule[ 'ids' ] );
