...
$single_image_label 	= __( 'Single image', 'quick-featured-images-pro' );
$random_images_label	= __( 'Random images', 'quick-featured-images-pro' );
...
<h3><?php _e( 'Default featured images for future posts', 'quick-featured-images-pro' ); ?></h3>
<div id="qfi_page_description">
	<p><?php echo $this->get_page_description(); ?>. <?php _e( 'Define the rules to use images as default featured images automatically every time a post is saved.', 'quick-featured-images-pro' ); ?></p>
	<p><?php _e( 'To use a rule choose the image and set both the taxonomy and the value. A rule which is defined only partially will be ignored.', 'quick-featured-images-pro' ); ?></p>
</div>

<?php 
if ( ! ( current_theme_supports( 'post-thumbnails' ) and current_theme_supports( 'post-formats' ) ) ) {
?>
<h3 style="margin-bottom:0"><?php _e( 'Notice', 'quick-featured-images-pro' ); ?></h3>
<div class="qfi-failure">
<?php
	if ( ! current_theme_supports( 'post-thumbnails' ) ) {
?>
	<p><?php _e( 'The current theme does not support featured images. Anyway you can use this plugin. The rules are stored and will be visible in a theme which supports featured images.', 'quick-featured-images-pro' ); ?></p>
<?php 
	}
	if ( ! current_theme_supports( 'post-formats' ) ) {
?>
	<p><?php _e( 'The current theme does not support post formats. Anyway you can use this plugin. The rules are stored and will be visible in a theme which supports post formats.', 'quick-featured-images-pro' ); ?></p>
<?php 
	}
?>
</div>
...
<form method="post" action="">
	...
	<h4><?php _e( 'Set rules', 'quick-featured-images-pro' ); ?></h4>
	<table class="widefat">
		<thead>
			<tr>
		...
		<tbody>
			<tr id="row_1" class="alternate">
				...
				<td>
					<p>
						<label><input type="checkbox" name="overwrite_automatically" value="1"<?php checked( isset( $this->selected_rules[ 'overwrite_automatically' ] ), '1' ); ?>><?php _e( 'Activate to automatically overwrite an existing featured image while saving a post', 'quick-featured-images-pro' ); ?></label>
					</p>
					<p class="description"><?php _e( 'If activated the rule is used automatically while saving a post to overwrite an existing featured image with the new one based on the following rules. Do not use this if you want to keep manually set featured images.', 'quick-featured-images-pro' ); ?></p>
				</td>
			...
			<tr id="row_2">
				...
				<td>
<?php
foreach ( $this->valid_first_image_options as $value => $label ) {
?>
...
?>
					<p class="description"><?php _e( 'If activated the rule is used automatically while saving a post to set the first image - if available in the media library or from external server - as the featured image of the post. If an image was not found the next rules will be applied.', 'quick-featured-images-pro' ); ?></p>
				</td>
				<td style="text-align: initial;">
				</td>
			</tr>
...
?>
					...
					<input type="text" name="rules[<?php echo $c; ?>][searchterm]" id="searchterm_<?php echo $c; ?>" value="<?php if ( 'searchterm' == $rule[ 'taxonomy' ] ) { echo esc_attr( $rule[ 'matchterm' ] ); } ?>">
				</td>
...
?>
					...
					<label for="searchterm_<?php echo $c; ?>"><?php echo $searchterm_label; ?> (<?php echo $search_desc; ?>)</label>:<br />
					<input type="text" name="rules[<?php echo $c; ?>][searchterm]" id="searchterm_<?php echo $c; ?>" value="">
				</td>
				<td><input type="button" name="remove_rule_<?php echo $c; ?>" value="X" class="button remove_rule" id="remove_rule_<?php echo $c; ?>"></td>
			</tr>
<?php
...
?>
					...
					<input type="text" name="rules[XX][searchterm]" id="searchterm_XX" value="">
				</td>
	...
	<h4><?php _e( 'Set rules order', 'quick-featured-images-pro' ); ?></h4>
	<p><?php _e( 'Set the order of the rules by selecting the number in the selection fields. 1 means in the first place, 2 means in the second place etc. If some rules have the same number it is unforeseeable in which order they will be applied. So set the order uniquely.', 'quick-featured-images-pro' ); ?></p>
	<p><?php _e( 'Regardless of the order in the list the rules are applied in the following order until a rule and a property of the post fit together:', 'quick-featured-images-pro' ); ?></p>
...
<?php } // foreach ( rules_order ) ?>
		<li><span><?php _e( 'no featured image.', 'quick-featured-images-pro' ); ?></span></li>
	</ol>
	<p><?php _e( 'Bear in mind that if two or more rules with the same taxonomy would fit to the post it is unforeseeable which image will become the featured image.', 'quick-featured-images-pro' ); ?></p>
<?php 
...
?>
	...
	<input type="hidden" id="confirmation_question" name="confirmation_question" value="<?php _e( 'Are you sure to remove this rule?', 'quick-featured-images-pro' ); ?>" />
