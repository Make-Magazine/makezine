	...
	if ( !empty( $count ) ) {
		foreach ( $steps as $idx => $step ) {
			...
			if ( isset( $images[0]->text ) ) {
				if ( function_exists( 'wpcom_vip_get_resized_remote_image_url' ) ) {
					echo '<img src="' . wpcom_vip_get_resized_remote_image_url( make_projects_to_s3( $images[0]->text ), 620, 465 ) .'" alt="' . esc_attr( the_title('', '', false ) ) . '" class=" main ' . esc_attr( $images[0]->imageid ) . ' ' . esc_attr( $images[0]->orderby ) .'" style="width:100%;height:auto;" />';
				} else {
					echo '<img src="' . wpcom_vip_get_resized_remote_image_url( make_projects_to_s3( $images[0]->text ), 620, 465 ) . '" data-loc="js-step-' . esc_attr( $step->number ) . ' alt="' . esc_attr( the_title('', '', false ) ) . '" class="main ' . esc_attr( $images[0]->imageid ) . ' ' . esc_attr( $images[0]->orderby ) .'" style="width:100%;height:auto;" />';
				}
			}
			...
			if ( !empty( $images[1]->text ) ) {
				...
				foreach ($images as $image) {
					...
					echo ( !empty($image->text ) ) ? '<img src="' . wpcom_vip_get_resized_remote_image_url( make_projects_to_s3( $image->text ), 140, 110 ) .'"  data-loc="js-step-' . esc_attr( $step->number ) . '" alt="' . esc_attr( the_title('', '', false ) ) . '" class="thumbs ' . esc_attr( $image->imageid ) . ' ' . esc_attr( $image->orderby ) .'" style="width:100%;height:auto;" />' : null ;
					echo '</span>';
				}
				echo '</span><!--.row-->';
			}
